...
def get_datasets(datasetname, **kwargs):
    invTrans = None
    data_root = os.environ.get('FL_BENCHMARK_DATA_ROOT', os.path.expanduser('~/data'))
    if datasetname == "Mnist":
        ...
        testset.targets = testset.targets.clone().detach()
    elif datasetname == "FashionMnist":
        transform = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.1307,), (0.3081,))
        ])
        trainset = torchvision.datasets.FashionMNIST(root=data_root, train=True,
                                                     download=True, transform=transform)
        testset = torchvision.datasets.FashionMNIST(root=data_root, train=False,
                                                    download=True, transform=transform)
