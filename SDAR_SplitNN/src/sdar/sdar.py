...
class SDARAttacker:
    ...
    def run(self, level, num_iter, config, fg_lr=0.001, u_shape=False, conditional=True, use_e_dis=True, use_d_dis=True, model_type="resnet", width="standard", diff_simulator=False, dropout=0.0, l1=0.0, l2=0.0, alpha=0.0, verbose_freq=100):
        ...
        self.intermidiate_shape = self.f.output_shape[1:]
        ...
        for i, ((x,y), (xs,ys)) in enumerate(iterator):
            ...
            # reset metrics
            self.fg_acc.reset_state()
            self.eg_acc.reset_state()
            self.e_dis_real_acc.reset_state()
            self.e_dis_fake_acc.reset_state()
            self.d_dis_real_acc.reset_state()
            self.d_dis_fake_acc.reset_state()
            ...
            self.log["fg_acc"][i] = float(self.fg_acc.result())
            self.log["eg_loss"][i] = np.mean(eg_loss)
            self.log["eg_acc"][i] = float(self.eg_acc.result())
            ...
            self.log["e_dis_real_acc"][i] = float(self.e_dis_real_acc.result())
            self.log["e_dis_fake_loss"][i] = np.mean(e_dis_fake_loss)
            self.log["e_dis_fake_acc"][i] = float(self.e_dis_fake_acc.result())
            ...
            self.log["d_dis_real_acc"][i] = float(self.d_dis_real_acc.result())
            self.log["d_dis_fake_loss"][i] = np.mean(d_dis_fake_loss)
            self.log["d_dis_fake_acc"][i] = float(self.d_dis_fake_acc.result())
